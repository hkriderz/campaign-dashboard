...
function compareCanvasserNames(a: string, b: string): number {
...
function tableToRecords(table: string[][], headerRowIndex = 0): { columns: string[]; records: Record<string, string>[] } {
...
function headerLookup(columns: string[]): Map<string, string> {
  return new Map(columns.map((column) => [canonical(column), column]));
}

function getValue(row: Record<string, string>, lookup: Map<string, string>, aliases: string[]): string {
...
function parseDateTime(raw: string): { iso: string | null; confidence: number; warning?: string } {
...
function detectReportDate(events: CanvassingKnockEvent[]): string | null {
...
function buildKnockEvents(parsedFiles: CanvassingParsedFile[]): {
...
function buildGapAnalysis(events: CanvassingKnockEvent[]): {
    ...
    for (let index = 0; index < sorted.length - 1; index++) {
      ...
      if (gapMinutes > NORMAL_GAP_MINUTES) {
        ...
        const detail: CanvassingGapDetail = {
          ...
          isBigGap: gapMinutes > BIG_GAP_MINUTES,
  ...
  gapDetails.sort((a, b) => b.gapMinutes - a.gapMinutes || a.canvasserName.localeCompare(b.canvasserName));
  bigGapDetails.sort((a, b) => b.gapMinutes - a.gapMinutes || a.canvasserName.localeCompare(b.canvasserName));

  return { canvasserStats, gapDetails, bigGapDetails };
}
